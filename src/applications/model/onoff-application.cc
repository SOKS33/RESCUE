...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
//UP modification
    .AddAttribute ("UserPriority", "The User Priority: UP_NC UP_VO UP_VI UP_CL UP_EE UP_BE UP_BK",
                   UintegerValue (0),
                   MakeUintegerAccessor (&OnOffApplication::m_up),
                   MakeUintegerChecker<uint8_t> ())
//UP modification end
//random send time modification
    .AddAttribute ("RandSendTimeLimit", 
                   "Limits the percentage randomization of the send time of the next schedulded packet.\
                    Value 0 means that transmit times will be constant. In other way this times will be modified by random +/- %.\
                    This parameter gives the max percent of such modification",
                   UintegerValue (0),
                   MakeUintegerAccessor (&OnOffApplication::m_randSendTimeLimit),
                   MakeUintegerChecker<uint32_t> ())
//random send time modification end
...
{
  NS_LOG_FUNCTION (this << ", UP: " << int(m_up));  //UP modification
      ...
      if (m_randSendTimeLimit == 0)
      ...
      else
        {
          nextTime = (Seconds ( (bits /
                               ...
                               * ((100-m_randSendTimeLimit) + (rand () % (2*m_randSendTimeLimit)))/100 ) ); // Time till next packet
        }
//random send time modification end
...
void OnOffApplication::SendPacket ()
{
  ...
  packet->AddPacketTag (QosTag (m_up)); //UP modification
  m_txTrace (packet);
  ...
  if (InetSocketAddress::IsMatchingType (m_peer))
    {
      NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
                   ...
                   << " total Tx " << m_totBytes << " bytes" //UP modification
                   << " user priority " << int(m_up)); //UP modification
    }
