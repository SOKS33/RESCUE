...
void
EnqueueTrace (std::string context, uint32_t node, uint32_t iff, Ptr<const Packet> pkt, const RescueMacHeader &hdr)
{
  std::cout << Simulator::Now ().GetSeconds () << "\tENQUEUE, TX to: \t" << hdr.GetDestination () << "\t" << *pkt << std::endl;
}

void
AckTimeoutTrace (std::string context, uint32_t node, uint32_t iff, Ptr<const Packet> pkt, const RescueMacHeader &hdr)
{
  std::cout << Simulator::Now ().GetSeconds () << "\tACK TIMEOUT, TX to: \t" << hdr.GetDestination () << "\t" << *pkt << std::endl;
}

void
SendDataDoneTrace (std::string context, uint32_t node, uint32_t iff, Mac48Address dest, bool success)
{
  std::cout << Simulator::Now ().GetSeconds () << "\tTX to: \t" << dest << "\t" << (success ? "OK" : "FAIL") << std::endl;
}
...
/* ============= Applications ============ */
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/Mac/Enqueue", MakeCallback (&EnqueueTrace));
  Config::Connect ("/NodeList/*/DeviceList/*/Mac/SendDataDone", MakeCallback (&SendDataDoneTrace));
  Config::Connect ("/NodeList/*/DeviceList/*/Mac/AckTimeout", MakeCallback (&AckTimeoutTrace));
