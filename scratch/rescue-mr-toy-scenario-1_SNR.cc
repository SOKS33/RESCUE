...
int main(int argc, char *argv[]) {
    ...
    //LogComponentEnable("RescueMac", LOG_LEVEL_ALL); //comment to switch off logging
    //LogComponentEnable("AdhocRescueMac", LOG_LEVEL_ALL); //comment to switch off logging
    ...
    //LogComponentEnable("RescueArqManager", LOG_LEVEL_ALL); //comment to switch off logging
    ...
    std::cout << snrSD << "\t" << snrSR << "\t" << snrRD << "\t"
            ...
            << ((m_send > m_received) ? (100 * ((double) m_send - (double) m_received) / (double) m_send) : 0.0) << "\t"
