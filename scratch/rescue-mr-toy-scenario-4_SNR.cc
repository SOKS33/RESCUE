...
/* Nodes 2 and 3 periodically transmits UDP packet to Node 1 using Node 0 as relay
 *
 *      Node 2 (Source 1)
 *       (0, y2, 0).
 *            \      ''--..        
...
int main (int argc, char *argv[])
{
  ...
  uint32_t sourceDataPhyRate1 = 6;
  uint32_t sourceDataPhyRate2 = 6;
...
/* ======= Command Line parameters ======= */
  ...
  cmd.AddValue ("xS1", "X-coordinate [m] of node 2 (source 1)", xS1);
  cmd.AddValue ("yS1", "Y-coordinate [m] of node 2 (source 1)", yS1);
  cmd.AddValue ("xS2", "X-coordinate [m] of node 3 (source 2)", xS2);
  cmd.AddValue ("yS2", "Y-coordinate [m] of node 3 (source 2)", yS2);

  cmd.AddValue ("snrRD", "SNR [dB] for direct transmission between node 0 (relay) and node 1 (destination)", snrRD);
  cmd.AddValue ("snrS1R", "SNR [dB] for direct transmission between node 2 (source 1) and node 0 (relay)", snrS1R);
  cmd.AddValue ("snrS2R", "SNR [dB] for direct transmission between node 3 (source 2) and node 0 (relay)", snrS2R);
  cmd.AddValue ("snrS1D", "SNR [dB] for direct transmission between node 2 (source 1) and node 1 (destination)", snrS1D);
  cmd.AddValue ("snrS2D", "SNR [dB] for direct transmission between node 3 (source 2) and node 1 (destination)", snrS2D);
  ...
  cmd.AddValue ("sourceDataPhyRate1", "Transmission Rate [Mbps] for Data Packets used by source 1 node", sourceDataPhyRate1);
  cmd.AddValue ("sourceDataPhyRate2", "Transmission Rate [Mbps] for Data Packets used by source 2 node", sourceDataPhyRate2);
...
/* ============ Nodes creation =========== */  
  ...
  NodeContainer nodes;
  nodes.Create (2);
  NodeContainer sourceNode1;
  sourceNode1.Create (1);
  NodeContainer sourceNode2;
  sourceNode2.Create (1);
...
/* ======== Positioning / Mobility ======= */
  ...
  positionAlloc->Add (Vector (xS1, yS1, 0.0));
  positionAlloc->Add (Vector (xS2, yS2, 0.0));
  mobility.SetPositionAllocator (positionAlloc);
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility.Install (NodeContainer (nodes, sourceNode1, sourceNode2));
...
/* ===== Channel / Propagation Model ===== */
  ...
  lossModel->SetLoss (nodes.Get(0)->GetObject<MobilityModel>(), nodes.Get(1)->GetObject<MobilityModel>(), TxPower - noise - snrRD); //Loss = TxPower - noise - snr
  lossModel->SetLoss (nodes.Get(0)->GetObject<MobilityModel>(), sourceNode1.Get(0)->GetObject<MobilityModel>(), TxPower - noise - snrS1R); //Loss = TxPower - noise - snr
  lossModel->SetLoss (nodes.Get(0)->GetObject<MobilityModel>(), sourceNode2.Get(0)->GetObject<MobilityModel>(), TxPower - noise - snrS2R); //Loss = TxPower - noise - snr
  lossModel->SetLoss (sourceNode1.Get(0)->GetObject<MobilityModel>(), nodes.Get(1)->GetObject<MobilityModel>(), TxPower - noise - snrS1D); //Loss = TxPower - noise - snr
  lossModel->SetLoss (sourceNode2.Get(0)->GetObject<MobilityModel>(), nodes.Get(1)->GetObject<MobilityModel>(), TxPower - noise - snrS2D); //Loss = TxPower - noise - snr
  lossModel->SetLoss (sourceNode1.Get(0)->GetObject<MobilityModel>(), sourceNode2.Get(0)->GetObject<MobilityModel>(), TxPower - CsPowerThr - 0.1); //Loss = TxPower - CsPowerThr
...
/* ======= PHY & MAC configuration ======= */  
  ...
  RescueHelper sourceRescue1;
  sourceRescue1.SetRemoteStationManager ("ns3::ConstantRateRescueManager",
							             "DataMode", StringValue (SimulationHelper::MapPhyRateToString (sourceDataPhyRate1)),
							             "ControlMode", StringValue (SimulationHelper::MapPhyRateToString (basicPhyRate)),
                                         "DataCw", UintegerValue (cwMin),
								         "MaxSrc", UintegerValue (++retryLimit));
  NetDeviceContainer sourceDevice1 = sourceRescue1.Install (sourceNode1, rescueChan, rescuePhy, rescueMac);

  RescueHelper sourceRescue2;
  sourceRescue2.SetRemoteStationManager ("ns3::ConstantRateRescueManager",
							             "DataMode", StringValue (SimulationHelper::MapPhyRateToString (sourceDataPhyRate2)),
							             "ControlMode", StringValue (SimulationHelper::MapPhyRateToString (basicPhyRate)),
                                         "DataCw", UintegerValue (cwMin),
								         "MaxSrc", UintegerValue (++retryLimit));
  NetDeviceContainer sourceDevice2 = sourceRescue2.Install (sourceNode2, rescueChan, rescuePhy, rescueMac);
  
  nodes.Add (sourceNode1);
  nodes.Add (sourceNode2);
  devices.Add (sourceDevice1);
  devices.Add (sourceDevice2);
...
/* ============= Applications ============ */
  ...
  NodeContainer sources (sourceNode1);
  sources.Add (sourceNode2);
  ...
  for (std::map< FlowId, FlowMonitor::FlowStats >::iterator flow = stats.begin (); flow != stats.end (); flow++)
    {
      ...
      //std::cout << "snrRD [dBm]:\tsnrS1R [dBm]:\tsnrS2R [dBm]:\tsnrS1D [dBm]:\tsnrS2D [dBm]:\tthr [Mbps]:\tloss [%]:\tdel [ms]:\tjitter [ms]:" << std::endl;
      //std::cout << snrRD << "\t" << snrS1R << "\t" << snrS2R << "\t" << snrS1D << "\t" << snrS2D << "\t" << 8000 * (double)flow->second.rxBytes / (flow->second.timeLastRxPacket - flow->second.timeFirstTxPacket).GetNanoSeconds () << std::endl;
    //}
      ...
      //std::cout << "snrRD [dBm]:\tsnrS1R [dBm]:\tsnrS2R [dBm]:\tsnrS1D [dBm]:\tsnrS2D [dBm]:\tthr [Mbps]:\tloss [%]:\tdel [ms]:\tjitter [ms]:" << std::endl;
      std::cout << snrRD << "\t" << snrS1R << "\t" << snrS2R << "\t" << snrS1D << "\t" << snrS2D << "\t" 
