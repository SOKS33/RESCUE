...
int main(int argc, char *argv[]) {
    ...
    double dSR = 200;
    double dRD = 200;
    double dSR_to_dSD = 1.0;
    double dRD_to_dSD = 1.0;
    ...
    //cmd.AddValue ("dSR", "distance [m] between node 0 (source) and node 2 (relay)", dSR);
    //cmd.AddValue ("dRD", "distance [m] between node 2 (relay) and node 1 (destination)", dRD);
    cmd.AddValue("dSR_to_dSD", "ratio of distances: source to relay and source to destination", dSR_to_dSD);
    cmd.AddValue("dRD_to_dSD", "ratio of distances: relay to destination and source to destination", dRD_to_dSD);
    ...
    double xR = (dSD * dSD + dSR * dSR - dRD * dRD) / (2 * dSD);
    double yR = sqrt(dSR * dSR - xR * xR);
