...
int main (int argc, char *argv[])
{
  ...
  double dSR1D = 400;
  double dSR1_to_dR1D = 1.0;
  double dSR2D = 400;
  double dSR2_to_dR2D = 1.0;
...
/* ======= Command Line parameters ======= */
  ...
  cmd.AddValue ("dSR1D", "total (sum) distance [m] of source to relay 1 and relay 1 to destination", dSR1D);
  cmd.AddValue ("dSR1_to_dR1D", "ratio of distances: source to relay 1 and relay 1 to destination", dSR1_to_dR1D);
  cmd.AddValue ("dSR2D", "total (sum) distance [m] of source to relay 2 and relay 2 to destination", dSR2D);
  cmd.AddValue ("dSR2_to_dR2D", "ratio of distances: source to relay 2 and relay 2 to destination", dSR2_to_dR2D);
...
/* ======== Positioning / Mobility ======= */
  ...
  dSR1 = dSR1D * (dSR1_to_dR1D / (1+dSR1_to_dR1D) );
  dR1D = dSR1D / dSR1_to_dR1D;
  dSR2 = dSR2D * (dSR2_to_dR2D / (1+dSR2_to_dR2D) );
  dR2D = dSR2D / dSR2_to_dR2D;
